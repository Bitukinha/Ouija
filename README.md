...
entra numa sessão como um *processo* concorrente disputando dois recursos compartilhados —
...
| "aging" da fila de prioridade | Prevenção de inanição (*starvation*) |
| Mutex desligado + duas escritas próximas | **Condição de corrida** (race condition) |
| Letra errada | *Page fault* (penalidade) |
...
- [Postgres via Neon](https://neon.tech) como banco — toda a lógica de jogo (mutex,
  ...
  só pelo servidor
- Sincronização multiplayer por *polling* (~900ms) — sem WebSocket, compatível com
